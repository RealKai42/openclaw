...
- Agents/session contextTokens after model switch: stop inheriting stale `contextTokens` from the prior model's session entry when switching to a different model; use `resolveContextTokensForModel` with a model-match guard so the context window resets correctly on model switch rather than staying stuck at the previous model's (potentially lower) limit. (#35372)
