...
- Plugins/symlink discovery: follow symlinks in extension directory scanning so `ln -s ~/my-plugin ~/.openclaw/extensions/my-plugin` development setups are discovered correctly; `Dirent.isDirectory()` returns false for symlinks causing silent skips. (#36754)
