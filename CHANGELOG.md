...
- Models/kimi-coding provider config: honor user-configured `baseUrl` and `api` over built-in defaults when auto-registering the kimi-coding provider, so custom endpoint overrides in `models.providers.kimi-coding` are not silently discarded. (#36353)
