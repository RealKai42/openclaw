...
- Feishu/groupPolicy alias: accept `"allowall"` as an alias for `"open"` in `groupPolicy` so users who set `groupPolicy: "allowall"` get the expected "allow all groups" behavior instead of silently falling back to `"allowlist"` and blocking group messages. (#36312)
