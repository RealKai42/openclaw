...
async function pruneIfNeeded(filePath: string, opts: { maxBytes: number; keepLines: number }) {
  ...
  // mode 0o600: run-log may contain job output/summaries; restrict to owner only (#35881)
...
export async function appendCronRunLog(
  ...
  const next = prev
    ...
    .then(async () => {
      ...
      // mode 0o600 on first create; mode is ignored when file already exists (#35881)
